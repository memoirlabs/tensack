# sixpack Rust Backend Architecture

**Document purpose:** define the complete backend structure for `sixpack`: a human-readable, TSV-backed local database with a tiny imported Rust-macro schema file, compile-time generated Rust registries, type-safe table/field/lookup APIs, and SDK generation from a shared schema IR.

**Status:** background reference. Current decisions live in `sixpack_book.md`
and the focused `sixpack_*_spec.md` files.

**Current chunk naming update:** any older examples in this document that mention
`active.6` or four-digit sealed segments are superseded by
`sixpack_chunk_naming_spec.md`: table chunks use reverse lowercase base-36 paths
like `zz/zzz.6`, `zz/zzy.6`, and generation folders keep chunk filenames at
3 characters.
...
```txt
schema.sixpack              small imported schema component
↓
Rust schema script        imports macros + includes schema.sixpack
↓
...
The user-facing schema stays extremely basic. `schema.sixpack` is Rust macro input, but it is not the full schema program; a normal Rust schema script imports sixpack macros and includes it.
...
Storage:
  human-readable .6 table data as source of truth
  internal transactions/logs/sync/indexes in sixpack.toml and .btf
...
The visible schema should stay small and declarative. `schema.sixpack` is a macro component included by a Rust schema script, not a separate custom DSL and not the whole generator.

Example `schema.sixpack`:
...
Internal engine fields do not belong in `.6` rows. They live in
`sixpack.toml` and `.btf` state:
...
```txt
sixpack-schema   defines schema macro input, validation, and IR
sixpack-codegen  generates Rust schema and registry types
sixpack-core     shared primitive definitions and typed handles
sixpack-engine   runtime storage engine
sixpack-cli      human tools
```
...
`schema.sixpack` is included by a real Rust schema script:
...
fn main() {
    ...
    sixpack_codegen::generate_rust(schema, "src/generated/schema.rs").unwrap();
}
```
...
fn main() {
    ...
    sixpack_codegen::run_schema_script("build_schema.rs").unwrap();
}
```
...
impl sixpack_core::TableSpec for Users {
...
pub mod users_fields {
    pub const ID: sixpack_core::Field<super::Users, String> =
        sixpack_core::Field::new("id");

    pub const EMAIL: sixpack_core::Field<super::Users, String> =
        sixpack_core::Field::new("email");

    pub const NAME: sixpack_core::Field<super::Users, String> =
        sixpack_core::Field::new("name");

    pub const AGE: sixpack_core::Field<super::Users, i64> =
        sixpack_core::Field::new("age");
}

pub mod users_lookups {
    pub const ID: sixpack_core::Lookup<super::Users, String> =
        sixpack_core::Lookup::new("id");

    pub const EMAIL: sixpack_core::Lookup<super::Users, String> =
        sixpack_core::Lookup::new("email");
}
```
...
```rust
pub static REGISTRY: sixpack_core::Registry = sixpack_core::Registry {
    tables: &[
        sixpack_core::TableDef {
            id: 1,
            name: "users",
            fields: &[
                sixpack_core::FieldDef {
                    name: "id",
                    ty: sixpack_core::SackType::Id,
                    required: true,
                },
                sixpack_core::FieldDef {
                    name: "email",
                    ty: sixpack_core::SackType::Text,
                    required: true,
                },
                sixpack_core::FieldDef {
                    name: "age",
                    ty: sixpack_core::SackType::Int,
                    required: true,
                },
            ],
            lookups: &[
                sixpack_core::LookupDef {
                    name: "id",
                    fields: &["id"],
                    unique: true,
                },
...
```rust
let db = sixpack_engine::Db::open("./data", &REGISTRY)?;
```

On startup, the engine checks:

```txt
sixpack.toml schema hash matches generated schema hash
all table folders exist
all .btf index files exist or can be rebuilt
active.6 is valid up to final full line
...
table's binary sixpack file (`.btf`) as rebuildable index state.
...
The `.6` files are the human-readable data source of truth.
...
```txt
email:a@test.com -> users/0000.6 offset=0 len=36 tx=1
```
...
```txt
a@test.com  0000.6  0   36  1
b@test.com  0000.6  37  34  2
```
...
.6 row segments are the source of truth
```
...
5. Serialize one .6 row in physical header order
6. Append to active.6
7. fsync depending on durability mode
8. Update id lookup in .btf
9. Update declared lookup indexes in .btf
10. Update sixpack.toml/checkpoint state when needed
```
...
Before compaction, the readable `.6` file may contain superseded rows:
...
`sixpack.toml` tracks compact readable engine state:
...
recover active.6 if needed
```

`sixpack.toml` is not the source of data truth and not a second schema.
...
From the same IR, generate Rust types and registry modules. Keep generated code downstream of the macro declarations in `schema.sixpack` so runtime code does not hand-model schema shapes.
...
```rust
impl sixpack_engine::Db {
    pub fn users(&self) -> sixpack_engine::TableHandle<Users> {
        self.table::<Users>()
    }

    pub fn messages(&self) -> sixpack_engine::TableHandle<Messages> {
        self.table::<Messages>()
    }
}
```
...
```txt
schema.sixpack changes
↓
sixpack check detects mismatch
↓
...
```txt
a@test.com	0000.6	0	93	1
b@test.com	0000.6	94	88	2
```
...
declared lookups are explicit in schema.sixpack
```
...
Sealed `.6` segment names are four digits: `0000.6` through `9999.6`.
Do not put more than 10,000 sealed segments in one folder. If a table grows past
`9999.6`, start a new group folder such as `g0002/` and begin again at
`0000.6`.
...
`sixpack.toml` is not schema truth. It is a compact physical map for fast open,
...
`schema.sixpack` and `.6` files if needed.
...
Define one central error type in `sixpack-core` or `sixpack-engine`.
...
schema hash mismatch: run sixpack check or sixpack upgrade
```
...
```txt
schema.sixpack macro declarations -> schema.ir.json -> all SDKs
```
...
Go type handwritten
schema.sixpack exists separately
```
...
```txt
1. schema.sixpack macro surface
2. schema validator
3. schema IR JSON output
4. Rust codegen for structs + registry
5. .6 append-only table writer
6. id lookup in .btf
7. get by id
...
rebuild lookups
compact table
sixpack.toml schema hash check
```
...
typed table handles
  ↓
.6 row segments as source of truth
  ↓
...
`sixpack` is a tiny local database that stores canonical data as human-readable TSV-style row files, uses explicit rebuildable lookup tables for speed, and compiles a small imported `schema.sixpack` component into type-safe Rust registries and generated SDKs.
