# Tensack Chunk Naming: Reverse-Sorted 3-Character Chunk Files

## Purpose

This document defines a very small chunk naming system for Tensack-style `.ten` files.
...
- The row format inside each `.ten` file is not part of this system
...
46,654             0              46,654          zz/001.ten
46,655             0              46,655          zz/000.ten

46,656             1              0               zy/zzz.ten
46,657             1              1               zy/zzy.ten
46,658             1              2               zy/zzx.ten

93,312             2              0               zx/zzz.ten
93,313             2              1               zx/zzy.ten
```
...
Once `zx/zzz.ten` exists, `zx/` sorts above `zy/` and `zz/`.
...
```ts
chunkPath(0);      // zz/zzz.ten
chunkPath(1);      // zz/zzy.ten
chunkPath(2);      // zz/zzx.ten
chunkPath(3);      // zz/zzw.ten

chunkPath(46655);  // zz/000.ten
chunkPath(46656);  // zy/zzz.ten
chunkPath(46657);  // zy/zzy.ten

chunkPath(93312);  // zx/zzz.ten
```
...
```rust
assert_eq!(chunk_path(0).unwrap(), "zz/zzz.ten");
assert_eq!(chunk_path(1).unwrap(), "zz/zzy.ten");
assert_eq!(chunk_path(2).unwrap(), "zz/zzx.ten");
assert_eq!(chunk_path(46655).unwrap(), "zz/000.ten");
assert_eq!(chunk_path(46656).unwrap(), "zy/zzz.ten");
assert_eq!(chunk_path(93312).unwrap(), "zx/zzz.ten");
```
...
```txt
data/messages/<generation>/<chunk>.ten
```

Example:

```txt
data/messages/zz/zzz.ten
data/messages/zz/zzy.ten
data/messages/zz/zzx.ten
```
