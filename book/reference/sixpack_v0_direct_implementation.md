...
`sixpack_book.md` and the focused `sixpack_*_spec.md` files. Examples in this
...
The final app binary should **not** expand `schema.sixpack` at runtime. It should use generated Rust code.
...
```rust id="7cwrm5"
use generated::schema::{open_sixpack, users};

fn main() -> sixpack_engine::Result<()> {
    let mut db = open_sixpack("./data")?;
...
[dependencies]
sixpack-engine = { path = "../crates/sixpack-engine" }

[build-dependencies]
sixpack-codegen = { path = "../crates/sixpack-codegen" }
...
```txt id="w0ur0s"
sixpack-codegen is build-time only.
sixpack-schema is build-time only.
sixpack-engine is runtime.
...
fn main() {
    ...
    sixpack_codegen::generate_rust_from_file(
        "schema.sixpack",
        "src/generated/schema.rs",
    )
    .expect("failed to generate sixpack schema");
}
```
...
pub fn open_sixpack(path: impl AsRef<std::path::Path>) -> Result<Database> {
...
pub mod users {
    ...
    impl RowCodec for Row {
        ...
        fn encode_put(&self, tx: u64, out: &mut String) {
            ...
            out.push_str("{\"_tx\":");
            sixpack_engine::push_u64(out, tx);

            out.push_str(",\"_op\":\"put\"");

            out.push_str(",\"id\":");
            sixpack_engine::push_json_str(out, &self.id);

            out.push_str(",\"email\":");
            sixpack_engine::push_json_str(out, &self.email);

            out.push_str(",\"name\":");
            sixpack_engine::push_json_str(out, &self.name);

            out.push_str(",\"age\":");
            sixpack_engine::push_i64(out, self.age);

            out.push_str(",\"score\":");
            sixpack_engine::push_f64(out, self.score);

            out.push_str(",\"active\":");
            sixpack_engine::push_bool(out, self.active);

            out.push('}');
        }

        fn decode_put_line(line: &str) -> Result<Self> {
            let op = sixpack_engine::json_get_string(line, "_op")?;

            if op != "put" {
                return Err(sixpack_engine::Error::BadRow);
            }

            Ok(Self {
                id: sixpack_engine::json_get_string(line, "id")?,
                email: sixpack_engine::json_get_string(line, "email")?,
                name: sixpack_engine::json_get_string(line, "name")?,
                age: sixpack_engine::json_get_i64(line, "age")?,
                score: sixpack_engine::json_get_f64(line, "score")?,
                active: sixpack_engine::json_get_bool(line, "active")?,
            })
        }
...
`crates/sixpack-engine/src/lib.rs`:
...
It is a **canonical sixpack JSONL parser** for objects the engine writes itself.
...
fn real_main() -> sixpack_engine::Result<()> {
    let mut args = std::env::args().skip(1);

    let cmd = args.next().ok_or(sixpack_engine::Error::BadRow)?;

    match cmd.as_str() {
        "put" => {
            let root = args.next().ok_or(sixpack_engine::Error::BadRow)?;
            let table = args.next().ok_or(sixpack_engine::Error::BadRow)?;
            let json = args.next().ok_or(sixpack_engine::Error::BadRow)?;
        ...
        "get" => {
            let root = args.next().ok_or(sixpack_engine::Error::BadRow)?;
            let table = args.next().ok_or(sixpack_engine::Error::BadRow)?;
            let id = args.next().ok_or(sixpack_engine::Error::BadRow)?;
        ...
        "get-by" => {
            let root = args.next().ok_or(sixpack_engine::Error::BadRow)?;
            let table = args.next().ok_or(sixpack_engine::Error::BadRow)?;
            let lookup = args.next().ok_or(sixpack_engine::Error::BadRow)?;
            let key = args.next().ok_or(sixpack_engine::Error::BadRow)?;
        ...
        _ => Err(sixpack_engine::Error::BadRow),
    }
}
```
...
```rust id="d1mz0e"
pub fn cli_put(root: &str, table: &str, json: &str) -> sixpack_engine::Result<()> {
    let mut db = open_sixpack(root)?;
...
pub fn cli_get(root: &str, table: &str, id: &str) -> sixpack_engine::Result<()> {
    let mut db = open_sixpack(root)?;
...
pub fn cli_delete(root: &str, table: &str, id: &str) -> sixpack_engine::Result<()> {
    let mut db = open_sixpack(root)?;
...
impl users::Row {
    ...
    pub fn decode_external_json(line: &str) -> sixpack_engine::Result<Self> {
        Ok(Self {
            id: sixpack_engine::json_get_string(line, "id")?,
            email: sixpack_engine::json_get_string(line, "email")?,
            name: sixpack_engine::json_get_string(line, "name")?,
...
```txt
1. Real schema.sixpack macro input handling in crates/sixpack-schema.
...
The current folder is intentionally small. The hot runtime path is already represented by `sixpack-engine/src/lib.rs` and `examples/chat/src/generated/schema.rs`.
