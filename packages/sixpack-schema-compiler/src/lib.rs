...
pub fn emit_raw_rust(ir: &SchemaIr) -> String {
    let mut out = String::new();
    out.push_str("pub mod sixpack_generated_schema {\n");
    out.push_str(&format!(
        "    pub const SCHEMA_HASH: &str = \"{}\";\n\n",
        ir.schema_hash()
    ));
    out.push_str("    pub trait GeneratedTables {\n");
    for table in &ir.tables {
        out.push_str(&format!(
    ...
    out.push_str("    }\n\n");
    out.push_str("    impl GeneratedTables for sixpack::Database {\n");
    for table in &ir.tables {
        out.push_str(&format!(
            "        fn {}(&self) -> {}::TableHandle<'_> {{\n",
            table.name, table.name
    ...
    for table in &ir.tables {
        ...
        out.push_str("\n        impl Row {\n");
        out.push_str(
            "            pub fn into_record(self) -> Result<sixpack::Record, sixpack::SchemaError> {\n",
        );
        out.push_str("                let mut record = sixpack::Record::new(NAME);\n");
        for field in &table.fields {
            if field.name == "id" {
        ...
        out.push_str("            }\n\n");
        out.push_str(
            "            pub fn from_record(record: &sixpack::Record) -> Result<Self, sixpack::SchemaError> {\n",
        );
        out.push_str("                if record.table() != NAME {\n");
        out.push_str(
            "                    return Err(sixpack::SchemaError::UnknownTable(record.table().to_owned()));\n",
        );
        out.push_str("                }\n");
        ...
        out.push_str("            }\n");
        out.push_str("        }\n");
        out.push_str("\n        pub fn table_schema() -> sixpack::TableSchema {\n");
        out.push_str("            let mut table = sixpack::TableSchema::new(NAME);\n");
        for field in &table.fields {
            out.push_str(&format!(
                "            table.add_field(\"{}\", sixpack::PrimitiveType::{:?}).unwrap();\n",
                field.name, field.ty
            ));
        }
        for lookup in &table.lookups {
        ...
        out.push_str("            fields: ::std::collections::BTreeMap<String, sixpack::Value>,\n");
        out.push_str("        }\n\n");
        ...
        out.push_str("                pub value: sixpack::Value,\n");
        out.push_str("            }\n\n");
        let id_field = table
            .fields
            .iter()
        ...
        out.push_str("            inner: sixpack::GetPage,\n");
        out.push_str("        }\n\n");
        ...
        out.push_str("            PageSelector { inner: sixpack::GetPage::new(NAME) }\n");
        out.push_str("        }\n\n");
        out.push_str("        pub fn count() -> CountSelector {\n");
        out.push_str("            CountSelector { inner: sixpack::GetCount::table(NAME) }\n");
        out.push_str("        }\n\n");
        out.push_str("        pub fn add(row: Row) -> sixpack::WriteChange {\n");
        out.push_str("            sixpack::WriteChange::add_record(record_from_row(row))\n");
        out.push_str("        }\n\n");
        out.push_str("        pub fn set(row: Row) -> sixpack::WriteChange {\n");
        out.push_str("            sixpack::WriteChange::set_record(record_from_row(row))\n");
        out.push_str("        }\n\n");
        out.push_str(
            "        pub fn edit(target: key::Key, patch: Patch) -> sixpack::WriteChange {\n",
        );
        out.push_str("            sixpack::WriteChange::edit(NAME, target.lookup, target.value, patch.fields)\n");
        out.push_str("        }\n\n");
        out.push_str("        pub fn remove(target: key::Key) -> sixpack::WriteChange {\n");
        out.push_str(
            "            sixpack::WriteChange::remove(NAME, target.lookup, target.value)\n",
        );
        out.push_str("        }\n");

        out.push_str("\n        pub struct TableHandle<'a> {\n");
        out.push_str("            db: &'a sixpack::Database,\n");
        out.push_str("        }\n\n");
        out.push_str("        impl<'a> TableHandle<'a> {\n");
        out.push_str("            pub fn new(db: &'a sixpack::Database) -> Self {\n");
        ...
        out.push_str("                }\n");
        out.push_str("            }\n\n");
        out.push_str("            pub fn put(&self, row: Row) -> Result<sixpack::AppendResult, sixpack::DatabaseError> {\n");
        ...
        out.push_str("\n        fn record_from_row(row: Row) -> sixpack::Record {\n");
        out.push_str(
            "            row.into_record().expect(\"generated rows only contain schema fields\")\n",
        );
        out.push_str("        }\n");

        out.push_str("\n        fn rows_from_records(records: Vec<sixpack::Record>) -> Result<Vec<Row>, sixpack::SchemaError> {\n");
    ...
    out.push_str("\n    pub fn database_schema_ref() -> &'static sixpack::DatabaseSchema {\n");
    out.push_str("        static SCHEMA: ::std::sync::OnceLock<sixpack::DatabaseSchema> = ::std::sync::OnceLock::new();\n");
    out.push_str("        SCHEMA.get_or_init(build_database_schema)\n");
    out.push_str("    }\n\n");
    out.push_str("    pub fn database_schema() -> sixpack::DatabaseSchema {\n");
    out.push_str("        database_schema_ref().clone()\n");
    out.push_str("    }\n\n");
    out.push_str("    fn build_database_schema() -> sixpack::DatabaseSchema {\n");
    out.push_str("        let mut schema = sixpack::DatabaseSchema::new();\n");
    for table in &ir.tables {
        out.push_str(&format!(
...
fn emit_by_constructor(out: &mut String, field: &FieldIr, is_id: bool, unique: bool) {
    ...
    out.push_str(&format!(
        "                super::{} {{ inner: sixpack::{}::new(super::NAME, \"{}\", {}) }}\n",
        selector,
        inner,
        name,
        rust_value_expr(field, "value")
...
fn emit_get_method(out: &mut String, field: &FieldIr, is_id: bool) {
    let name = if is_id { "id" } else { &field.name };
    out.push_str(&format!(
        "            pub fn {}(&self, value: {}) -> Result<Option<Row>, sixpack::DatabaseError> {{\n",
        name,
        rust_param_type(field)
    ));
    out.push_str(&format!(
...
fn emit_find_method(out: &mut String, field: &FieldIr) {
    out.push_str(&format!(
        "            pub fn {}(&self, value: {}) -> Result<Vec<Row>, sixpack::DatabaseError> {{\n",
        field.name,
        rust_param_type(field)
    ));
    out.push_str(&format!(
...
fn rust_value_expr(field: &FieldIr, value_expr: &str) -> String {
    match field.ty {
        PrimitiveType::Id => format!("sixpack::Value::Id({value_expr}.into())"),
        PrimitiveType::Text => format!("sixpack::Value::Text({value_expr}.into())"),
        PrimitiveType::Int => format!("sixpack::Value::Int({value_expr})"),
        PrimitiveType::Float => format!("sixpack::Value::Float({value_expr})"),
        PrimitiveType::Bool => format!("sixpack::Value::Bool({value_expr})"),
    }
}

fn rust_record_value_expr(field: &FieldIr) -> String {
    match field.ty {
        PrimitiveType::Id => format!("sixpack::Value::Id(self.{})", field.name),
...
fn rust_record_extract_expr(field: &FieldIr) -> String {
    ...
    format!(
        "match record.fields().get(\"{field}\") {{
                        Some(sixpack::Value::{variant}(value)) => {value_expr},
                        Some(value) => return Err(sixpack::SchemaError::TypeMismatch {{
                            table: NAME.to_owned(),
                            field: \"{field}\".to_owned(),
                            expected: sixpack::PrimitiveType::{expected:?},
                            found: value.value_type(),
                        }}),
                        None => return Err(sixpack::SchemaError::MissingField {{
                            table: NAME.to_owned(),
                            field: \"{field}\".to_owned(),
                        }}),
                    }}",
...
mod tests {
    ...
    fn compiles_schema_file() {
        let source = include_str!("../../sixpack/examples/chat_schema.sixpack");
    ...
    fn compiles_random_multi_table_schema_with_all_simple_types() {
        ...
        assert!(code.contains("pub trait GeneratedTables"));
        ...
        assert!(code.contains("::std::sync::OnceLock<sixpack::DatabaseSchema>"));
        assert!(!code.contains("pub fn insert(db: &sixpack::Database, row: Row)"));
    ...
    fn emits_row_structs() {
        ...
        assert!(code.contains("pub fn from_record(record: &sixpack::Record)"));
