//! Command-line layer for Tensack.
//!
...
pub fn run(args: impl IntoIterator<Item = String>) -> Result<(), CliError> {
    ...
    match args.next().as_deref() {
        Some("--version") | Some("-V") => {
            println!("tensack {}", env!("CARGO_PKG_VERSION"));
            Ok(())
        }
    ...
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::UnknownCommand(command) => {
                ...
                write!(formatter, "run `tensack help` for usage")
            }
        }
    }
}
...
fn print_help() {
    ...
    println!();
    println!("Usage:");
    println!("  tensack --version");
    println!("  tensack help");
}
