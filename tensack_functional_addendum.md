...
lookup-first reads
.ten TSV-style row segments as source truth
...
`schema.tensack` file wraps table blocks in one outer `schema! { ... }`, and a
...
the earlier generated examples used uppercase constants because that is common rust style for constants. do not do that for the public tensack interface.
...
```txt
set	Y3ZfMQ	bXNnXzE	50120	0	0000.ten	0	117	1
set	Y3ZfMQ	bXNnXzI	50120	0	0000.ten	118	121	2
del	Y3ZfMQ	bXNnXzI	50120	0	0000.ten	118	121	3
```
...
```txt
set	YUB0ZXN0LmNvbQ	dV8x	10492	0	0000.ten	0	83	1
```
...
the v1 goal is correctness and simplicity. `.ten` stays readable, while binary
...
```txt
del	b2xkQHRlc3QuY29t	dV8x	10492	0	0000.ten	0	72	2
set	bmV3QHRlc3QuY29t	dV8x	10492	0	0000.ten	73	73	2
```
...
```txt
del	bmV3QHRlc3QuY29t	dV8x	10492	0	0000.ten	73	73	3
del	dV8x	dV8x	10492	0	0000.ten	73	73	3
```
...
the canonical `.ten` row plus internal live/delete state still decides truth.
...
```txt
0000.ten has chunk id 0
0001.ten has chunk id 1
0002.ten has chunk id 2
```
...
hold writer lock
flush active.ten
fsync active.ten if durability requires it
rename active.ten to next four-digit sealed chunk
create new active.ten
rebuild lookup snapshots for that table
write tensack.toml tmp
rename tensack.toml tmp into place
release writer lock
```

because row pointers contain segment names, any operation that renames
`active.ten` must rebuild lookup snapshots. this is acceptable in v1.

later optimization:

```txt
write rows directly to numbered active chunk
make active.ten a stable manifest value
...
```txt
schema.tensack Rust macro surface
generated registry
...
that gives tensack the intended feel:
